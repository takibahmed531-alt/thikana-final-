...
export default function TopNav() {
  ...
  return (
    <header className="flex sticky top-0 z-50 w-full border-b border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md transition-colors duration-300">
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 h-16 flex items-center justify-between gap-4 sm:gap-6">
        {/* Brand Logo */}
        ...
        >
          ...
          <input
            ...
            className="w-full pl-10 pr-12 py-2 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-full text-sm text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 transition-colors duration-300"
          />
          {searchQuery && (
            <button
              type="submit"
              className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg transition-colors shadow-sm cursor-pointer"
              aria-label="Search"
            >
        ...
        <nav className="flex items-center gap-1.5 shrink-0">
          {/* Desktop-only Navigation Links & Auth Actions */}
          ...
          {/* Bilingual Language Switcher [ EN / বাং ] (Visible on Both Mobile & Desktop) */}
          <button
            type="button"
